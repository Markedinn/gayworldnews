  ...
  const waitForData = setInterval(() => {
    ...
    if (typeof globalData !== "undefined" && Object.keys(globalData).length > 0) {
      ...
      if (countryData) {
        ...
        const utilityStack = document.querySelector(".top-utility-stack");
        if (countryData.isAI && utilityStack) {
          // Check to prevent duplicate banners if the interval runs twice
          if (!document.getElementById("ai-warning-box")) {
            const aiBanner = document.createElement("div");
            aiBanner.id = "ai-warning-box";
            aiBanner.className = "ai-warning-banner";
            aiBanner.innerHTML = `
                            <span class="banner-label">⚠️ STATUS: PRELIMINARY</span>
            <p>
                <strong>AI-generated placeholder page.</strong><br>
                Human Writing Arriving by July 2026.<br>
                Check back soon - We think you'll love our work!
            </p>
                        `;
            utilityStack.appendChild(aiBanner);
          }
        }
